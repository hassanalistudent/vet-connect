...
import { useGetProfileQuery, useUpdateDoctorAvailabilityMutation } from "../../redux/api/userApiSlice";
import { 
  FaCalendarCheck, 
  FaStar, 
  FaUserMd, 
...
const DoctorDashboard = () => {
  ...
  const { data: dashboardData, isLoading: isDashboardLoading, error: dashboardError, refetch: refetchDashboard } = useGetDoctorDashboardQuery();
  ...
  const [updateAvailability, { isLoading: isUpdatingAvailability }] = useUpdateDoctorAvailabilityMutation();
  ...
  // Set initial availability status from profile - using availableNow
  useEffect(() => {
    if (doctorProfile?.availableNow !== undefined) {
      setAvailabilityStatus(doctorProfile.availableNow);
    }
  }, [doctorProfile?.availableNow]);
  ...
  // Handle availability toggle - using availableNow
  const handleAvailabilityToggle = async () => {
    try {
      const newStatus = !availabilityStatus;
      
      // Use availableNow to match schema
      ...
      toast.success(`You are now ${newStatus ? 'available' : 'unavailable'} for appointments`);
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between">
              ...
              <div className="mt-4 md:mt-0 flex items-center space-x-3">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center backdrop-blur-sm ${
                    availabilityStatus 
                ...
                >
                  {isUpdatingAvailability ? (
                    ...
                    <FaToggleOn className="w-5 h-5 mr-2" />
                  ) : (
                    <FaToggleOff className="w-5 h-5 mr-2" />
                  )}
